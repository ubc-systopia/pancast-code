...
/// Packet send timer
static sl_simple_timer_t packet_timer;
 ...
 *****************************************************************************/
static void throughput_peripheral_on_packet_timer_rise(sl_simple_timer_t *timer,
                                                     void *data)
{
  (void) data;
  (void) timer;
  ...
  if (send_transmission_on && !notification_sent) {
    ...
    if (!indication_sent) {
      ...
      if (indication_confirmed || indication_timer_rised) {
        ...
        throughput_peripheral_on_finish(peripheral_state.throughput, peripheral_state.count);
      }
    }
  }
}
...
static void handle_throughput_peripheral_start(bool send_transmission_on)
{
  ...
  if (peripheral_state.mode == THROUGHPUT_MODE_FIXED_TIME) {
    ...
    app_assert_status(sc);
    sc = sl_simple_timer_start(&packet_timer,
                                   3,
                                   throughput_peripheral_on_packet_timer_rise,
                                   NULL,
                                   true);
    app_assert_status(sc);
  }
