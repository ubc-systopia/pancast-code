...
#include "platform/throughput_central_system.h"
...
static stat_t rx_packet_interval;
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_characteristic_value_id:
      ...
      if (evt->data.evt_gatt_characteristic_value.characteristic == indications_handle
        ...
        uint32_t ms = sl_sleeptimer_tick_to_ms(ticks);
        uint32_t stat_ms = ms - last_packet_time_ms;
//        app_log_info("%u\r\n", stat_ms);
        if (last_packet_time_ms != 0) {
          stat_add(stat_ms, rx_packet_interval);
          if (stat_ms > rx_packet_interval.max) {
            rx_packet_interval.max = stat_ms;
          }
        } else {
         // rx_packet_interval.max = stat_ms;
          rx_packet_interval.min = stat_ms;
        }
        if (stat_ms < rx_packet_interval.min) {
          rx_packet_interval.min = stat_ms;
        }
    //    app_log_info("n: %.0f\r\n", rx_packet_interval.n);
        last_packet_time_ms = ms;
...
static void reset_variables(void)
{
  ...
  memset(&rx_packet_interval, 0, sizeof(stat_t));
}
...
void throughput_central_scanning_start(void)
{
  ...
  app_log_info("Scanning started..." APP_LOG_NEW_LINE_RN);
...
//  app_log_info(THROUGHPUT_UI_TIME_FORMAT APP_LOG_NEW_LINE, ((int)time));
  app_log_info("on central finish\r\n");
  app_log_info("%s (%s): %.0f, %f, %f\r\n", "Inter packet time", "ms",
		 rx_packet_interval.n, rx_packet_interval.mu, rx_packet_interval.sigma);
}
