...
void app_test(bool start)
{
  ...
  if (start) {
    // Start test
    app_log_info("Starting test..." APP_LOG_NEW_LINE_RN);
 ...
 * Timer callback
 *****************************************************************************/
void app_test_timer_callback(sl_simple_timer_t *timer, void *data)
{
  (void) data;
  (void) timer;
  button_timer_rised = true;
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      app_log("Throughput Test initialized" APP_LOG_NEW_LINE_RN);
  //    if (app_check_buttons()) {
        // Enable throughput test in Central mode if button is pressed
        app_log_info("Central mode set." APP_LOG_NEW_LINE_RN);
...
//        throughput_peripheral_enable();
 //     }
    ...
    case sl_bt_evt_connection_opened_id:
      app_log_info("Connection opened" APP_LOG_NEW_LINE_RN);
    ...
    case sl_bt_evt_connection_closed_id:
      app_log_info("Connection closed" APP_LOG_NEW_LINE_RN);
      break;
								  ...
								  stat_t pkt_stats)
{
  ...
  app_log_info("%s (%s): %.0f, %f, %f\r\n", "Inter packet time", "ms",
		  pkt_stats.n, pkt_stats.mu, pkt_stats.sigma);
  app_log_info("max: %u, min: %u \r\n", pkt_stats.max, pkt_stats.min);
  throughput_ui_set_throughput(throughput);
  throughput_ui_set_count(count);
  throughput_ui_update();
}
