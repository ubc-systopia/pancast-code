...
# Experiment setup for short distance advertisement tests

distance_permutations = ['2m']  # ['0.minus20db', '1m', '2m', '4m']

# REMEMBER: 29 is actually 20 here. experiment was modified but file names not
signal_permutations = ['4db']  # ['minus40db', 'minus29db', 'minus10db', '4db']
variations = ['parallel', 'perp']
folders = ['google', 'samsung']
parameters = ['rssi', 'time', 'rssi_delta']
        ...
        for signal in signal_permutations:
            for distance in distance_permutations:
                for variation in variations:
                    name = f"{folder}/raw_samsung_{distance}_{signal}_nogaen_{variation}.txt"  # temporary change
                    names.append(name)
    return names
            ...
            for distance in distance_permutations:
                for variation in variations:
                    for data_type in parameters:
                        header = f"{folder}_{signal}_{distance}_{variation}_{data_type}"
