...
sl_status_t
sl_iostream_eusart_init_vcom (void);
...
sl_iostream_instance_info_t sl_iostream_instance_vcom_info =
  { .handle = &sl_iostream_vcom.stream, .name = "vcom", .type =
      SL_IOSTREAM_TYPE_UART, .periph_id = SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO,
...
sl_iostream_eusart_init_vcom (void)
{
  ...
  sl_iostream_eusart_config_t config_vcom =
    { .eusart = SL_IOSTREAM_EUSART_VCOM_PERIPHERAL, .flow_control =
        SL_IOSTREAM_EUSART_VCOM_FLOW_CONTROL_TYPE, .enable_high_frequency =
        SL_IOSTREAM_EUSART_VCOM_ENABLE_HIGH_FREQUENCY, .clock =
...
#endif
        .tx_port = SL_IOSTREAM_EUSART_VCOM_TX_PORT, .tx_pin =
            SL_IOSTREAM_EUSART_VCOM_TX_PIN, .rx_port =
            SL_IOSTREAM_EUSART_VCOM_RX_PORT, .rx_pin =
            SL_IOSTREAM_EUSART_VCOM_RX_PIN,
#if defined(SL_IOSTREAM_EUSART_VCOM_CTS_PORT)
        .cts_port = SL_IOSTREAM_EUSART_VCOM_CTS_PORT, .cts_pin =
            SL_IOSTREAM_EUSART_VCOM_CTS_PIN,
#endif
#if defined(SL_IOSTREAM_EUSART_VCOM_RTS_PORT)
        .rts_port = SL_IOSTREAM_EUSART_VCOM_RTS_PORT, .rts_pin =
            SL_IOSTREAM_EUSART_VCOM_RTS_PIN,
#endif
      };
  sl_iostream_uart_config_t uart_config_vcom =
    { .tx_irq_number = SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(
        SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO), .rx_irq_number =
        SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL_NO),
        .rx_buffer = rx_buffer_vcom, .rx_buffer_length =
            SL_IOSTREAM_EUSART_VCOM_RX_BUFFER_SIZE, .lf_to_crlf =
...
void
sl_iostream_eusart_init_instances (void)
{
...
sl_iostream_eusart_vcom_sleep_on_isr_exit (void)
{
