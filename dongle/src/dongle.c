...
sl_sleeptimer_timer_handle_t *led_timer_handle;
...
void dongle_led_notify() {
  ...
  sc = sl_sleeptimer_stop_timer(led_timer_handle);
  if (sc != SL_STATUS_OK) {
    log_errorf("failed period led timer stop, sc: %d\r\n", sc);
  }
  sc = sl_sleeptimer_restart_periodic_timer_ms(led_timer_handle, LED_TIMER_MS/4,
