...
void update_risk_data(void* new_risk_data, int len) {
  // check length of new risk data
   if (len > risk_data_len) {
       // handle reallocating array
       printf("larger than risk size\r\n");
   }
   ...
   sl_status_t sc = sl_bt_advertiser_set_data(advertising_set_handle, 8, PER_ADV_SIZE,
                                               &risk_data[0]);
...
void app_process_action(void)
{
  ...
  // can put this on a timer as well
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      sc = sl_bt_advertiser_set_timing(
        ...
        75,  // min. adv. interval (milliseconds * 1.6) orig. 160
